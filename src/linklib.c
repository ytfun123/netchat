...
/* usb_configuration_descriptor_t and usb_endpoint_descriptor_t, as defined
 * in usbdrvce.h, get compiler-inserted padding (10 bytes instead of 9, and
 * 8 instead of 7, respectively) because they contain a uint16_t field.
 * That padding ends up embedded in the literal bytes sent over the wire,
 * which breaks USB descriptor parsing on standards-strict hosts (confirmed
 * on both Windows and ChromeOS -- the interface enumerates fine, but zero
 * endpoints are ever reported). These are byte-exact, packed drop-in
 * replacements used only for constructing the static descriptor tables. */
#pragma pack(push, 1)
typedef struct {
    uint8_t  bLength;
    ...
    uint8_t  bMaxPower;
} packed_configuration_descriptor_t;
...
__attribute__((constructor)) static void link_Init(void) {
    ...
    #pragma pack(push, 1)
    static struct {
        packed_configuration_descriptor_t configuration;
        struct {
            usb_interface_descriptor_t interface;
            packed_endpoint_descriptor_t endpoints[2];
        } interface0;
    } configuration1 = {
        .configuration = {
    ...
    #pragma pack(pop)
    _Static_assert(sizeof(configuration1) == 9 + 9 + 7 + 7, "descriptor layout still has padding!");
    static const usb_configuration_descriptor_t *configurations[] = {
        (const usb_configuration_descriptor_t *)&configuration1.configuration,
    };
    ...
    if (usb_Init(&event_handler, NULL, &standard, USB_DEFAULT_INIT_FLAGS) == USB_SUCCESS)
        return;
